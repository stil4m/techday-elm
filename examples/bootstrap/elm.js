...
Elm.Index.make = function (_elm) {
   ...
   var renderUser = function (user) {
      return $Html$Shorthand.tr_(_U.list([$Html$Shorthand.td_(_U.list([$Html.text(user.name)]))
                                         ,$Html$Shorthand.td_(_U.list([function (_p0) {    return $Html.text($Basics.toString(_p0));}(user.age)]))]));
   };
   var renderUsers = function (users) {    return A2($List.map,renderUser,users);};
   var view = F2(function (actions,model) {
      return $Html$Shorthand.div_(_U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Elm Content")]))
                                          ,$Bootstrap$Html.tableStriped_(_U.list([$Html$Shorthand.thead_(_U.list([$Html$Shorthand.tr_(_U.list([$Html$Shorthand.th_(_U.list([$Html.text("Name")]))
                                                                                                                                              ,$Html$Shorthand.th_(_U.list([$Html.text("Age")]))]))]))
                                                                                 ,$Html$Shorthand.tbody_(renderUsers(model.users))]))]));
   });
   var update = F2(function (action,model) {
      ...
      if (_p1.ctor === "NoOp") {
            ...
            return _U.update(model,{users: A2($List._op["::"],_p1._0,model.users)});
         }
   });
   var NewUser = function (a) {    return {ctor: "NewUser",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var actionMailbox = $Signal.mailbox(NoOp);
   var UserRecord = F2(function (a,b) {    return {name: a,age: b};});
   var addUser = Elm.Native.Port.make(_elm).inboundSignal("addUser",
   "Maybe.Maybe Index.UserRecord",
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && "name" in v && "age" in v ? {_: {}
                                                                                                                                        ,name: typeof v.name === "string" || typeof v.name === "object" && v.name instanceof String ? v.name : _U.badPort("a string",
                                                                                                                                        v.name)
                                                                                                                                        ,age: typeof v.age === "number" && isFinite(v.age) && Math.floor(v.age) === v.age ? v.age : _U.badPort("an integer",
                                                                                                                                        v.age)} : _U.badPort("an object with fields `name`, `age`",
      v));
   });
   var onAddUser = A2($Signal.map,function (_p2) {    return A2($Maybe.withDefault,NoOp,A2($Maybe.map,NewUser,_p2));},addUser);
   var initialModel = {users: _U.list([])};
   var model = A3($Signal.foldp,update,initialModel,A2($Signal.merge,actionMailbox.signal,onAddUser));
   var userCount = Elm.Native.Port.make(_elm).outboundSignal("userCount",
   function (v) {
      return v;
   },
   A2($Signal.map,$List.length,A2($Signal.map,function (_) {    return _.users;},model)));
   var main = A2($Signal.map,view(actionMailbox.address),model);
