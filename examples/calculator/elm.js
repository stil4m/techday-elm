...
Elm.Index.make = function (_elm) {
   ...
   var _op = {};
   var asOperation = F2(function (x,o) {
      return A2($Basics.flip,
      function () {
         var _p0 = o;
         switch (_p0.ctor)
         {case "Add": return F2(function (x,y) {    return x + y;});
            case "Minus": return F2(function (x,y) {    return x - y;});
            case "Multiply": return F2(function (x,y) {    return x * y;});
            case "Divide": return F2(function (x,y) {    return x / y;});
            default: return $Basics.always($Basics.identity);}
      }(),
      x);
   });
   var nextOperation = function (m) {
      return A2($Maybe.withDefault,
      m.previousOperation,
      $Maybe.oneOf(_U.list([A3($Maybe.map2,asOperation,m.value,m.operand),A2($Maybe.map,asOperation(m.previousValue),m.operand)])));
   };
   var handleOperand = F2(function (o,m) {
      var _p1 = o;
      if (_p1.ctor === "Assign") {
            var n = nextOperation(m);
            return _U.update(m,{value: $Maybe.Nothing,operand: $Maybe.Nothing,previousValue: n(m.previousValue),previousOperation: n});
         } else {
            var previousValue = A2($Maybe.withDefault,
            A2($Maybe.withDefault,m.previousValue,m.value),
            A2($Maybe.map,function (f) {    return f(m.previousValue);},A3($Maybe.map2,asOperation,m.value,m.operand)));
            return _U.update(m,{previousValue: previousValue,value: $Maybe.Nothing,operand: $Maybe.Just(_p1)});
         }
   });
   var handleNewNumber = F2(function (x,m) {
      var _p2 = m.value;
      if (_p2.ctor === "Nothing") {
            return _U.update(m,{value: $Maybe.Just($Basics.toFloat(x))});
         } else {
            return _U.update(m,{value: $Maybe.Just($Basics.toFloat(x) + _p2._0 * 10)});
         }
   });
   var update = F2(function (action,model) {
      var _p3 = action;
      if (_p3.ctor === "NewNumber") {
            return A2(handleNewNumber,_p3._0,model);
         } else {
            return A2(handleOperand,_p3._0,model);
         }
   });
   var basicButton = F3(function (act,t,addr) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("action-button")]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,addr,act)]),_U.list([$Html.text(t)]))]));
   });
   var screenValue = function (m) {    return $Basics.toString(A2($Maybe.withDefault,m.previousValue,m.value));};
   var screen = function (m) {    return A2($Html.div,_U.list([$Html$Attributes.$class("screen")]),_U.list([$Html.text(screenValue(m))]));};
   var model = {value: $Maybe.Nothing,operand: $Maybe.Nothing,previousOperation: $Basics.identity,previousValue: 0.0,inDecimal: false};
   var Model = F5(function (a,b,c,d,e) {    return {value: a,operand: b,previousOperation: c,previousValue: d,inDecimal: e};});
   var Operation = function (a) {    return {ctor: "Operation",_0: a};};
   var operandButton = F2(function (operand,op) {    return A2(basicButton,Operation(operand),op);});
   var NewNumber = function (a) {    return {ctor: "NewNumber",_0: a};};
   var numberButton = function (v) {    return A2(basicButton,NewNumber(v),$Basics.toString(v));};
   var Decimal = {ctor: "Decimal"};
   var Assign = {ctor: "Assign"};
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([screen(model)
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("buttons-container")]),
              _U.list([A2(numberButton,7,address)
                      ,A2(numberButton,8,address)
                      ,A2(numberButton,9,address)
                      ,A3(operandButton,Multiply,"*",address)
                      ,A2(numberButton,4,address)
                      ,A2(numberButton,5,address)
                      ,A2(numberButton,6,address)
                      ,A3(operandButton,Divide,"/",address)
                      ,A2(numberButton,1,address)
                      ,A2(numberButton,2,address)
                      ,A2(numberButton,3,address)
                      ,A3(operandButton,Minus,"-",address)
                      ,A3(operandButton,Decimal,".",address)
                      ,A2(numberButton,0,address)
                      ,A3(operandButton,Assign,"=",address)
                      ,A3(operandButton,Add,"+",address)]))]));
   });
