...
Elm.ProjectApi = Elm.ProjectApi || {};
Elm.ProjectApi.make = function (_elm) {
   "use strict";
   _elm.ProjectApi = _elm.ProjectApi || {};
   if (_elm.ProjectApi.values) return _elm.ProjectApi.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var Project = F2(function (a,b) {    return {key: a,name: b};});
   var project = A3($Json$Decode.object2,Project,A2($Json$Decode._op[":="],"key",$Json$Decode.string),A2($Json$Decode._op[":="],"name",$Json$Decode.string));
   var projects = $Json$Decode.list(project);
   var getProjects = function (f) {    return $Effects.task(A2($Task.map,f,$Task.toMaybe(A2($Http.get,projects,"http://localhost:8000/projects.json"))));};
   return _elm.ProjectApi.values = {_op: _op,getProjects: getProjects,Project: Project};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $ProjectApi = Elm.ProjectApi.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Increment": return {ctor: "_Tuple2",_0: _U.update(model,{counter: model.counter + 1}),_1: $Effects.none};
         case "Decrement": return {ctor: "_Tuple2",_0: _U.update(model,{counter: model.counter - 1}),_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: _U.update(model,{projects: _p0._0,gotAll: true}),_1: $Effects.none};}
   });
   var projectItem = function (project) {
      return A2($Html.li,
      _U.list([]),
      _U.list([$Html.text(A2($Basics._op["++"],project.name,A2($Basics._op["++"]," (",A2($Basics._op["++"],project.key,")"))))]));
   };
   var projectsList = function (projects) {    return A2($Html.ul,_U.list([]),A2($List.map,projectItem,projects));};
   var noProjects = A2($Html.div,_U.list([]),_U.list([A2($Html.em,_U.list([]),_U.list([$Html.text("We could not load any projects!")]))]));
   var Model = F3(function (a,b,c) {    return {counter: a,projects: b,gotAll: c};});
   var AllProjects = function (a) {    return {ctor: "AllProjects",_0: a};};
   var init = {ctor: "_Tuple2",_0: {counter: 1,projects: $Maybe.Nothing,gotAll: false},_1: $ProjectApi.getProjects(function (x) {    return AllProjects(x);})};
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrement)]),_U.list([$Html.text("-")]))
              ...
              ,A2($Maybe.withDefault,noProjects,A2($Maybe.map,projectsList,model.projects))]));
   });
