...
Elm.Utils.make = function (_elm) {
   ...
   return _elm.Utils.values = {_op: _op,onRightClick: onRightClick};
};
Elm.Box = Elm.Box || {};
Elm.Box.make = function (_elm) {
   ...
   var _op = {};
   var baseBox = _U.list([{ctor: "_Tuple2",_0: "border",_1: "1px solid gray"}
                         ,{ctor: "_Tuple2",_0: "border-radius",_1: "5px"}
                         ,{ctor: "_Tuple2",_0: "width",_1: "20px"}
                         ,{ctor: "_Tuple2",_0: "height",_1: "20px"}]);
   var bombBox = A2($Basics._op["++"],_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}]),baseBox);
   var markedBox = A2($Basics._op["++"],_U.list([{ctor: "_Tuple2",_0: "background",_1: "gray"}]),baseBox);
   var mark = function (model) {    return _U.update(model,{isMarked: $Basics.not(model.isMarked)});};
   var reveal = function (model) {    return model.isMarked ? model : _U.update(model,{isOpened: true});};
   var update = F2(function (action,model) {    var _p0 = action;if (_p0.ctor === "Reveal") {    return reveal(model);} else {    return mark(model);}});
   var makeMine = function (model) {    return _U.update(model,{isMine: true});};
   var init = function (isMine) {    return {isMine: isMine,isOpened: false,isMarked: false};};
   var Mark = {ctor: "Mark"};
   var Reveal = {ctor: "Reveal"};
   var view = F2(function (address,model) {
      return model.isOpened ? model.isMine ? A2($Html.div,
      _U.list([A2($Html$Attributes.attribute,"class","tile"),$Html$Attributes.style(bombBox)]),
      _U.list([A2($Html.img,
      _U.list([A2($Html$Attributes.attribute,
              "src",
              "http://vignette2.wikia.nocookie.net/tibia/images/a/a5/Ultimate_Explosion.gif/revision/latest?cb=20050528232132&path-prefix=en")
              ,A2($Html$Attributes.attribute,"width","18")
              ,A2($Html$Attributes.attribute,"height","18")]),
      _U.list([]))])) : A2($Html.div,
      _U.list([A2($Html$Attributes.attribute,"class","tile"),$Html$Attributes.style(baseBox)]),
      _U.list([function (_p1) {    return $Html.text($Basics.toString(_p1));}(model.isMine)])) : model.isMarked ? A2($Html.div,
      _U.list([$Html$Attributes.$class("tile marked")
              ,$Html$Attributes.style(markedBox)
              ,A2($Html$Events.onClick,address,Reveal)
              ,A2($Utils.onRightClick,address,Mark)]),
      _U.list([$Html.text("")])) : A2($Html.div,
      _U.list([$Html$Attributes.$class("tile"),$Html$Attributes.style(baseBox),A2($Html$Events.onClick,address,Reveal),A2($Utils.onRightClick,address,Mark)]),
      _U.list([]));
   });
   var Model = F3(function (a,b,c) {    return {isMine: a,isOpened: b,isMarked: c};});
   return _elm.Box.values = {_op: _op,init: init,makeMine: makeMine,update: update,view: view,Model: Model};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var updateGridItem = F3(function (id,action,model) {
      ...
      function (row) {
         ...
         function (_p0) {
            ...
            var _p3 = _p1._0;
            var _p2 = _p1._1;
            return _U.eq(_p3,id) ? {ctor: "_Tuple2",_0: _p3,_1: A2($Box.update,action,_p2)} : {ctor: "_Tuple2",_0: _p3,_1: _p2};
         },
         row);
      },
      model.grid)});
   });
   var decrement = function (model) {    return _U.update(model,{counter: model.counter + 1});};
   var increment = function (model) {    return _U.update(model,{counter: model.counter - 1});};
   var update = F2(function (action,model) {
      var _p4 = action;
      switch (_p4.ctor)
      {case "BoxAction": return A3(updateGridItem,_p4._0,_p4._1,model);
         case "Increment": return decrement(model);
         default: return increment(model);}
   });
   var initRow = F2(function (idOffset,length) {
      return A2($List.map,function (a) {    return {ctor: "_Tuple2",_0: idOffset + a,_1: $Box.init(true)};},_U.range(0,length - 1));
   });
   var initGrid = F2(function (width,height) {    return A2($List.map,function (h) {    return A2(initRow,h * width,width);},_U.range(0,height - 1));});
   var init = F2(function (width,height) {    return {grid: A2(initGrid,width,height),counter: 0};});
   var BoxAction = F2(function (a,b) {    return {ctor: "BoxAction",_0: a,_1: b};});
   var viewRow = F2(function (address,row) {
      return A2($Html.tr,
      _U.list([]),
      A2($List.map,
      function (_p5) {
         var _p6 = _p5;
         return A2($Html.td,_U.list([]),_U.list([A2($Box.view,A2($Signal.forwardTo,address,BoxAction(_p6._0)),_p6._1)]));
      },
      row));
   });
   var viewGrid = F2(function (address,grid) {    return A2($Html.table,_U.list([]),A2($List.map,viewRow(address),grid));});
   var Decrement = {ctor: "Decrement"};
   var Increment = {ctor: "Increment"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2(viewGrid,address,model.grid)
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrement)]),_U.list([$Html.text("-")]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(model.counter))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increment)]),_U.list([$Html.text("+")]))]));
   });
   var foo = Increment;
   var Model = F2(function (a,b) {    return {grid: a,counter: b};});
   var main = $StartApp$Simple.start({model: A2($Debug.watch,"Model",A2(init,5,5)),view: view,update: update});
